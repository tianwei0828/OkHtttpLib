...
/**
 * Created by tianwei on 16/10/10.
 */
public class BackgroundExecutor {
        ...
        for (int i = TASKS.size() - 1; i >= 0; i--) {
            ...
            if (id.equals(task.id)) {
                if (task.future != null) {
                    ...
                    Logger.w(TAG, "A task with id " + task.id + " cannot be cancelled (the executor set does not support it)");
                } else {
     ...
     * {@link WrongThreadListener#onUiExpected()} if it
     * doesn't.
     */
